...
const registro_usuario_admin = async function(req, res) {
    ...
    if (req.user) {
        ...
        if (usuarios.length >= 1) {
            res.status(200).send({data: undefined, message: 'Ya hay un usuario registrado con ese email.'})
        } else {
            bcrypt.hash('123456', null, null, async function(err, hash) {
                if (err) {
                    res.status(200).send({data: undefined, message: 'No se ha podido encriptar la contraseña.'})
                } else {
                    data.password = hash;
...
/**Utilizamos este método para enviar los campos necesarios para el registro (nombre, apellidos, e-mail...) */
// const registro_usuario_admin = async function(req, res) {
...
//         } else {
                /*Donde va '123456', iría data.password, pero hemos decidido que siempre que se cree un nuevo usuario, la
                contraseña por defecto va a ser '123456' */
//             bcrypt.hash('123456', null, null, async function(err, hash) {
